...
void LidDrivenCavity::initializeBoundaries(Simulation* sim)
{
    ...
    for (int t = 0; t < sim->m_settings.top.m_thickness; t++)
        for (int i = 1; i <= nParticles.x; i++)
            sim->m_boundaryParticles.emplace_back(sim->m_settings.top.m_particleMass, Vec2d(spacing.x * i, spacing.y * t + sim->m_settings.physicalSize.y), Vec2d(sim->m_settings.top.m_velocity.x, sim->m_settings.top.m_velocity.y));
