#include "scenario_fillingbucket.h"
#include "datastructures/vector.h"
#include "simulation/simulation.h"

void FillingBucket::initializeParticles(Simulation* sim)
{
    Vec2i nParticles;
...
void FillingBucket::initializeBoundaries(Simulation* sim)
{
    ...
    for (int i = 0; i < sim->m_settings.nParticles; i++)
    {
        sim->m_boundaryparticles.emplace_back(sim->m_settings.bottom.m_particleMass, Vec2d(-1), Vec2d(0));
    }
...
void FillingBucket::init(Simulation* sim)
{
    initializeParticles(sim);
    initializeBoundaries(sim);
}

void FillingBucket::update(Simulation* sim)
{
    static double time_elapsed = 0.0;
    time_elapsed += sim->m_settings.dt;
    if (time_elapsed >= m_fillrate && sim->m_particles.size() < sim->m_settings.nParticles-1)
    {
        // add particle
        double r = ((double)rand() / RAND_MAX);
        sim->m_particles.emplace_back(sim->m_settings.mass, Vec2d((sim->m_settings.physicalSize.x / 2.0) + r - 0.01, sim->m_settings.physicalSize.y), Vec2d(0, 0));
        sim->m_particles.emplace_back(sim->m_settings.mass, Vec2d((sim->m_settings.physicalSize.x / 2.0) + r + 0.01, sim->m_settings.physicalSize.y), Vec2d(0, 0));
        sim->m_particles.emplace_back(sim->m_settings.mass, Vec2d((sim->m_settings.physicalSize.x / 2.0) + r, sim->m_settings.physicalSize.y - 0.01), Vec2d(0, 0));
        sim->m_particles.emplace_back(sim->m_settings.mass, Vec2d((sim->m_settings.physicalSize.x / 2.0) + r, sim->m_settings.physicalSize.y + 0.01), Vec2d(0, 0));
        time_elapsed = 0.0;
        sim->m_boundaryparticles.erase(sim->m_boundaryparticles.end() - 1);
        sim->m_boundaryparticles.erase(sim->m_boundaryparticles.end() - 1);
        sim->m_boundaryparticles.erase(sim->m_boundaryparticles.end() - 1);
        sim->m_boundaryparticles.erase(sim->m_boundaryparticles.end() - 1);
    }
