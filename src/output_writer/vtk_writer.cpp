...
VtkWriter::VtkWriter(Grid& grid) :
m_grid(grid), fileNo(0)
{
...
void VtkWriter::writeFile(double currentTime)
{
    // Assemble the filename
    std::stringstream fileName;
    fileName << "out/output_" << std::setw(4) << setfill('0') << fileNo << "." << writer->GetDefaultFileExtension();

    // increment file number
    fileNo++;

    // assign the new file name to the output writer
    ...
    //writer->SetDataModeToAscii();     // comment this in to get ascii text files: those can be checked in an editor
    writer->SetDataModeToBinary();      // set file mode to binary files: smaller file sizes
